...
V4 = ROOT / "docs/lucy/production/v4/V4_READINESS.md"
...
def disposition(path: Path) -> tuple[str, str]:
    ...
    if "/v3/" in rel:
        if any(token in rel for token in ("/components/wings/", "/components/ribbons/", "/components/shards/", "/identity/")):
            return "reference-only", "flattened crop; not promoted as a standalone modular layer"
        return "approved", "guard-reviewed v3 registered source or deterministic derivative"
    if "/frontpage/variation-" in rel:
        ...
        return "approved-derived", "front-page derivative rebuilt from v3 or frozen atmosphere/authority"
...
BRIEF = [
    ...
    ("P0", "Primary homepage hero", "frontpage/variation-a/hero/lucy-primary.webp", "approved"),
    ("P0", "Alternate homepage hero", "frontpage/variation-a/hero/lucy-alternate.webp", "approved"),
    ("P0", "Zero full state", "states/lucy-zero.webp", "approved"),
    ("P0", "Heaven full state", "v3/states/lucy-heaven.webp", "approved"),
    ("P0", "Hell registered full state", "v3/states/lucy-hell.webp", "approved"),
    ("P0", "Ultra full state", "v3/states/lucy-ultra.webp", "approved"),
    ("P0", "Responsive hero pair", "frontpage/variation-a/hero/", "approved-derived"),
    ("P1", "Four-state panels", "frontpage/variation-a/states/panels/", "approved-derived"),
    ("P1", "State + neutral portraits", "frontpage/variation-a/portraits/", "approved-derived"),
    ...
    ("P2", "Shard + optical FX", "frontpage/variation-a/fx/", "approved-derived"),
    ("P2", "Background plates", "frontpage/variation-a/backgrounds/", "approved-derived"),
    ("P2", "Dividers + state icons", "frontpage/variation-a/identity/", "approved-derived"),
    ("P3", "OpenGraph 1200x630", "frontpage/variation-a/social/lucy-og-1200x630.webp", "approved-derived"),
    ("P3", "Social square 1080", "frontpage/variation-a/social/lucy-square-1080.webp", "approved-derived"),
    ("P3", "Social portrait 1080x1350", "frontpage/variation-a/social/lucy-portrait-1080x1350.webp", "approved-derived"),
    ("P3", "Story 1080x1920", "frontpage/variation-a/social/lucy-story-1080x1920.webp", "approved-derived"),
]
...
.panel,.guard,.brief-card{{padding:16px}} .pass{{color:#7fffb5}} .gap,.fail{{color:#ff7a93}} .reference-only{{color:#f7c76a}} .approved,.approved-derived{{color:#7fffb5}}
...
<header><h1>Lucy / approved stack</h1><p class="muted">Internal static artifact · PR #49 v3 source → PR #50 front-page derivatives → live prototype imports. No v2 character is approved here.</p></header>
<main><section class="summary">
<div class="panel"><strong>Registered bodies</strong><p>Heaven + Hell share geometry. Ultra recovered from its retained v3 raw; no new model call.</p></div>
<div class="panel"><strong>Frontend</strong><p>React prototype imports v3 masters directly and the focused authority katana pack. CSS inversion and duplicate tear overlays are removed.</p></div>
...
<div class="panel"><strong>V4 gate</strong><p class="pass">Not triggered by current full-body guards.</p><p>Read <code>docs/lucy/production/v4/V4_READINESS.md</code> if owner review rejects a character.</p></div>
</section>
...
{{name:'Heaven v3',status:'PASS',path:'packages/site/src/assets/lucy/v3/masters/lucy-heaven.webp',note:'One head; two arms/hands; two long traceable legs/feet; opaque skirt; one steel katana; both diamond eyes; cyan shard wings.'}},
{{name:'Hell v3',status:'PASS',path:'packages/site/src/assets/lucy/v3/masters/lucy-hell.webp',note:'Byte-identical alpha and geometry to Heaven; full foreground RGB inversion including skin; closed eyes; exactly one red tear.'}},
{{name:'Ultra v3',status:'PASS',path:'packages/site/src/assets/lucy/v3/masters/lucy-ultra.webp',note:'Recovered checker matte restores one head, hair, and gold wings. Two arms/hands, two traceable legs/feet, opaque skirt, two matching katanas.'}}];
const url=p=>rootPrefix+p; document.querySelector('#guards').innerHTML=guardData.map(g=>`<article class="guard"><span class="badge pass">${{g.status}}</span><h3>${{g.name}}</h3><img loading="lazy" src="${{url(g.path)}}" alt="${{g.name}}"><p>${{g.note}}</p><code>${{g.path}}</code></article>`).join('');
...
function draw(){{const q=search.value.toLowerCase();const rows=DATA.inventory.filter(x=>{{const statusOK=status.value==='all'||(status.value==='approved-set'&&x.status!=='reference-only')||x.status===status.value;return statusOK&&(source.value==='all'||sourceOf(x.path)===source.value)&&(`${{x.path}} ${{x.category}} ${{x.state}}`.toLowerCase().includes(q))}});count.textContent=`${{rows.length}} / ${{DATA.inventory.length}} assets`;gallery.innerHTML=rows.map(x=>`<article class="asset"><figure><img loading="lazy" src="${{url(x.path)}}" alt=""></figure><div class="asset-body"><span class="badge ${{x.status}}">${{x.status}}</span><span class="badge">${{x.state}}</span><span class="badge">${{x.category}}</span><p><code>${{x.path}}</code></p><p class="muted">${{x.width?`${{x.width}}×${{x.height}} · `:''}}${{x.note}}</p></div></article>`).join('')}}
[search,status,source].forEach(x=>x.addEventListener('input',draw));draw();</script></body></html>"""


def main() -> None:
    inventory = collect()
    approved = [item for item in inventory if item["status"] in {"approved", "approved-derived"}]
    references = [item for item in inventory if item["status"] == "reference-only"]
    manifest = {
        "version": "lucy-approved-brandkit/v1",
        "character_authority": "packages/site/src/assets/lucy/v3",
        "frontend_stack": "PR #49 v3 -> PR #50 frontpage",
        "policy": "v2 character assets excluded; reference-only flattened modular crops are not approved frontend inputs",
...
""")
    ...
    V4.write_text("""# Lucy v4 Readiness (Dormant)

Status: **not triggered by the current full-body guard review.**

If owner review rejects a v3 character, v4 is scoped to Lucy character pixels
only. Existing Zero/Heaven/Hell/Ultra background plates and front-page
atmosphere atlases remain frozen.

## Frozen v4 contract

1. Regenerate one complete Heaven registered master under the current anatomy,
   proportion, modesty, authority-katana, and transparency guards.
2. Derive Hell as the exact full foreground RGB inversion of Heaven, including
   skin; only a bounded closed-eyes plus exactly-one-red-tear edit may differ.
3. Regenerate one complete Ultra master with one head, two arms/hands, two long
   traceable legs/feet, opaque skirt coverage, and exactly two matching katanas.
4. Use a flat chroma field rather than a baked checker if true alpha is not
   returned. Validate normal and dark composites before promotion.
5. Rebuild only character-bearing hero/state/panel/portrait/mobile/social
   derivatives. Reuse all current background, FX, identity-vector, and katana
   authority assets.

No v4 generation call is authorized or made by this readiness note.
""")

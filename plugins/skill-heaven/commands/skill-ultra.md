...
it) and the depth, then call the `summon` tool with a `limit` between 1 and 5
that matches that choice. Print each returned card **verbatim** before using
...
- Do not add rung, token or savings numbers of your own. The counts on the line
  are PROVISIONAL and the crown rung has no fixed count at all.

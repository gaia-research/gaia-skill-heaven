...
These files are copied verbatim from the owner's supplied assets and are
rendered by `src/harnessMarks.tsx` for the shared door tiles and compatibility
row.

- `hermes.webp` — owner-supplied transparent Hermes raster (512×512 RGBA).
- `grok.svg` — owner-supplied light-background Grok mark, from
...
- `antigravity.svg` — official Google Antigravity mark (24×24 viewBox).

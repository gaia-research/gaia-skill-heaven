...
```bash
codex-zero                                      # interactive Codex TUI at the zero rung
codex-zero -- "Reply with OK"                   # headless: trailing args run `codex exec`
...
skills. It then spawns the interactive `codex` TUI, or `codex exec` when a
prompt or other trailing arguments are given; no shared Codex state is edited.
`--print` shows this composition without starting Codex. See [PROBE.md](PROBE.md).

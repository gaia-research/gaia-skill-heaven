...
Missing Gaia installability is UNKNOWN, not unreachable. Normal retrieval remains
offline; live API discovery is not a prerequisite for offline guarantees.
Tree's install parity operator tool explicitly must NOT run in workflows/CI (it is
a 40-60m standalone operator tool). Parity PASS != materializable; expected NO_SOURCE
refusal passes; comparator FAIL can coexist with successful materialization.
